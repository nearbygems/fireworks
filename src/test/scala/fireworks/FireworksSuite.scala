      ...
      (launched.countDown <= 0) ==> {
        launched.next match {
          case exploding: Exploding =>
            ...
            assert(exploding.particles.value.forall(_.color ==
                                                    launched.particlesColor), "'particles' should all be of the same color as the 'particlesColor' of the 'Launched' value")
            assert(exploding.particles.value.forall(_.position ==
                                                    launched.position), "'particles' should have the same 'position' as the 'position' of the 'Launched' value")
            Prop.passed
          case _ =>
  ...
  def checkProperty(prop: Prop): Unit =
    ...
    def failure(labels: Set[String], fallback: String): Nothing =
      if labels.isEmpty then {
        ...
        throw AssertionError(labels.mkString(". "))
      }
